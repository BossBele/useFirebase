...
import { DocumentData, getDocs, onSnapshot, query as queryFn, Query, QueryDocumentSnapshot, startAfter, queryEqual } from 'firebase/firestore';
...
export default function useCollection({
  ...
  const lastVisibleDoc = useRef<{[key: number]: QueryDocumentSnapshot<DocumentData, DocumentData>}>({});
  ...
  const constraints = useMemo(() => ({
    ...constraintsProp,
    ...(page > 1 ? { limit: page * (constraintsProp?.limit * 10) } : {}),
  }), [constraintsProp, page]);
  ...
  const query: Query = useMemo(() => {
    if (page > 1 && lastVisibleDoc.current?.[page]) {
      return queryFn(generateQuery(collection, constraints), startAfter(lastVisibleDoc.current));
    }
    return generateQuery(collection, constraints);
  }, [collection, constraints, page]);
  ...
  useEffectDebugger(() => {
    async function getCollection() {
      if (!isEnabled || queryEqual(query, currentQuery.current)) {
        return;
      }
      // check if firebase is initialized
      if (getApps().length < 1) {
      ...
      if (withRealtimeUpdates) {
        ...
        unsubscribe = onSnapshot(query, (querySnapshot) => {
          ...
          if (querySnapshot.docs.length) {
            // Get the last visible document
            lastVisibleDoc.current[page] = querySnapshot.docs[querySnapshot.docs.length - 1];
          }
        });
      } else {
    ...
    getCollection();
  }, [query, isEnabled, handleOnResults, page, withRealtimeUpdates]);
